...
public class CompareController {
    ...
    private void handleCompareSubmit() {
        try {
            ...
            JsonObject compareData = fetchCompareData(playerOne, playerTwo, opponentAbbr);

            JsonObject playerOneData = compareData.getAsJsonObject("player_one");
            JsonObject playerTwoData = compareData.getAsJsonObject("player_two");

            SessionManager.setLatestCompare(new SessionManager.CompareResult(
                    playerOneData.get("player").getAsString(),
                    playerTwoData.get("player").getAsString(),
                    opponentName,
                    getDouble(playerOneData, "ppg"),
                    getDouble(playerTwoData, "ppg"),
                    getDouble(playerOneData, "apg"),
                    getDouble(playerTwoData, "apg"),
                    getDouble(playerOneData, "rpg"),
                    getDouble(playerTwoData, "rpg"),
                    getDouble(playerOneData, "fg_pct"),
                    getDouble(playerTwoData, "fg_pct"),
                    getDouble(playerOneData, "mpg"),
                    getDouble(playerTwoData, "mpg"),
                    getInt(playerOneData, "games_played"),
                    getInt(playerTwoData, "games_played")
            ));

            if (SessionManager.isLoggedIn()) {
                try {
                    FirebaseService.saveCompareHistory(
                            SessionManager.getUid(),
                            SessionManager.getLatestCompare()
                    );
                    System.out.println("✅ Compare saved to Firebase");
                } catch (Exception e) {
                    e.printStackTrace();
                    System.out.println("❌ Failed to save compare history");
                }
            }
        ...
        try {
            ...
            for (Map<String, Object> search : history) {
                ...
                if (type.equals("compare")) {
                    String playerOne = String.valueOf(search.getOrDefault("playerOne", "Player 1"));
                    String playerTwo = String.valueOf(search.getOrDefault("playerTwo", "Player 2"));
                    String opponent = String.valueOf(search.getOrDefault("opponent", "Any Opponent"));

                    row = new Label("Compare: " + playerOne + " vs " + playerTwo + " • " + opponent);
                } else {
